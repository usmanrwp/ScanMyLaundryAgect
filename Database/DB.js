...
export default class DB {
  ...
  createTable = db => {
    return new Promise(resolve => {
      ...
      db.transaction(tx => {
        tx.executeSql(
          "CREATE TABLE IF NOT EXISTS Categories (ID INTEGER PRIMARY KEY AUTOINCREMENT,OID, Name, Picture, Type)"
        );
      });
      //Category Table End
      ...
      db.transaction(tx => {
        tx.executeSql(
          "CREATE TABLE IF NOT EXISTS DB_Version (ID INTEGER PRIMARY KEY AUTOINCREMENT, Version,Entity) "
        );
      });
      //DB Version End
      ...
      db.transaction(tx => {
        tx.executeSql(
          "CREATE TABLE IF NOT EXISTS Sub_Categories (ID INTEGER PRIMARY KEY AUTOINCREMENT,Item_Id,C_ID, Name, Picture, Detail,Price)"
        );
      });
